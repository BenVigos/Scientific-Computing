...
## Assignment 3: Incompressible flow around a cylinder (LBM, FDM, FEM) & WiFi signal optimization (DE)

Assignment 3 focuses on the Karman vortex street benchmark and compares three numerical methods:

- Lattice Boltzmann Method (LBM)
- Finite Difference Method (FDM)
- Finite Element Method (FEM)
- Differential Evolution (DE) for WiFi signal optimization

All Assignment 3 scripts are in `assignment_3/scripts/`.

### Scripts (`assignment_3/scripts`)

- `complete_example_lbm.py` - end-to-end LBM run and visualization workflow
- `complete_example_fdm.py` - end-to-end FDM run and visualization workflow
- `complete_example_fem.py` - end-to-end FEM run and visualization workflow
- `convergence_study.py` - quantitative comparison workflow (benchmark + sweeps, metrics, saved states)
- `plot_convergence_study.py` - plotting utility for convergence/accuracy/performance/physical metrics
- `lbm_quantitative_analysis.py` - additional LBM-centered quantitative analysis script
- `envirmonment.py` - assignment-local environment definitions/helpers for script runs
- `solvers.py` - assignment-local solver wrappers/variants used by scripts
- `visualization.py` - assignment-local plotting/video helper utilities
- `wifi.py` - DE-based WiFi signal optimization (saves coverage and optimization landscape plots)

### Outputs (`assignment_3/outputs`)

Assignment 3 figures, videos, and post-processed artifacts are written under `assignment_3/outputs/`.

Common output locations include:

- `assignment_3/outputs/convergence_study/` - plots from `plot_convergence_study.py`
- `assignment_3/outputs/` - media generated by complete example scripts (final-state images, videos)
- `assignment_3/data/` - plots generated by `wifi.py` (signal coverage, optimization landscape, etc.)

### Data products (`assignment_3/data`)

For convergence/quantitative runs, tabular and cached state data are stored in:

- `assignment_3/data/convergence_study/`
  - method CSV files (`lbm_convergence/results.csv`, `fdm_convergence/results.csv`, `fem_convergence/results.csv`)
  - per-run saved states (`*.npz`) and metadata (`*.json`)
  - benchmark cache files

### Typical run commands

```powershell
python assignment_3/scripts/complete_example_lbm.py
python assignment_3/scripts/complete_example_fdm.py
python assignment_3/scripts/complete_example_fem.py
python assignment_3/scripts/convergence_study.py
python assignment_3/scripts/plot_convergence_study.py
python assignment_3/scripts/wifi.py
```
